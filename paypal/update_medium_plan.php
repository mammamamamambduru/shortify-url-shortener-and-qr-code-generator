...
if (!isset($orderData['status']) || $orderData['status'] !== "COMPLETED") {
    die("<div class='alert alert-danger text-center'>The transaction is invalid or not completed.</div>");
}
...
try {
    ...
    $stmt = $conn->prepare("UPDATE users SET plan = 'Medium', remaining_links = 500 WHERE email = ?");
    ...
    $conn->rollback();
    die("<div class='alert alert-danger text-center'>Error processing transaction: " . $e->getMessage() . "</div>");
}
